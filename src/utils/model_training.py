...
mlflow_experiment_name = "LSTM_Training"  # Nome do experimento no MLflow
# ==========================
# Helpers
# ==========================
...
def evaluate_model(model, loader, device, scaler_y):
    ...
    mape = np.mean(np.abs((targets_orig - predictions_orig) / targets_orig)) * 100 if np.all(targets_orig != 0) else np.nan
    return {
        'mse': mse,
        'mae': mae,
        'rmse': rmse,
...
def main(
    ...
    with mlflow.start_run() as run:
        ...
        # Log losses per epoch to MLflow
        for epoch in range(len(history['train_losses'])):
            mlflow.log_metric("train_loss", history['train_losses'][epoch], step=epoch)
            mlflow.log_metric("val_loss", history['val_losses'][epoch], step=epoch)
        ...
        print("Avaliando modelo nos conjuntos de dados...")
        train_metrics = evaluate_model(model, train_loader, device, scaler_y)
        val_metrics = evaluate_model(model, val_loader, device, scaler_y)
        test_metrics = evaluate_model(model, test_loader, device, scaler_y)
        # Log train metrics
        ...
        mlflow.log_artifact(f"{final_model_path}.pth")
        mlflow.log_artifact(f"{final_model_path}_metadata.json")
